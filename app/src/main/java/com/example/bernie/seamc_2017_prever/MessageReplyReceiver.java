...
public class MessageReplyReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (Info_center.REPLY_ACTION.equals(intent.getAction())) {
            int conversationId = intent.getIntExtra(Info_center.CONVERSATION_ID, -1);
    ...
    private CharSequence getMessageText(Intent intent) {
        ...
        if (remoteInput != null) {
            return remoteInput.getCharSequence(Info_center.EXTRA_VOICE_REPLY);
        }
        return null;
    }
}
